...
1. Copy the `spot_ros2` to the Core I/O Payload using `scp`:
   ...
   ```bash
   scp -P 20022 -r /path/to/spot_ros2 spot@ROBOT_IP_ADDRESS:~/spot_ros2
   ```
...
5. Run the Docker container:
   ...
   ```bash
   docker run --privileged -v '/data:/data' -v /var/run/dbus/system_bus_socket:/var/run/dbus/system_bus_socket -v /dev:/dev --device /dev/video0 --device /dev/video1 --device /dev/video2 --network=host -it spot_ros2 bash
   ```
...
1. `ssh -p 20022 spot@ROBOT_IP_ADDRESS`
2. `nmcli d`
...
From the host,
```scp -P 20022 /path/to/local/file spot@ROBOT_IP_ADDRESS:~/path/to/remote/directory```
...
```scp -r -P 20022 spot@ROBOT_IP_ADDRESS:/host/path/target .```
...
3. `scp -P 20022 spot_ros2.tgz spot@ROBOT_IP_ADDRESS:~/path/to/remote/directory/spot_ros2.tgz`
4. `ssh -p 20022 spot@ROBOT_IP_ADDRESS`

On the Core I/O Payload,

1. `docker load -i spot_ros2.tgz`
2. `docker run --privileged -v '/data:/data' -v /var/run/dbus/system_bus_socket:/var/run/dbus/system_bus_socket -v /dev:/dev --device /dev/video0 --device /dev/video1 --device /dev/video2 --network=host -it spot_ros2 bash`

Where

* `-v /data:/data` mounts the directory into the docker from the host
